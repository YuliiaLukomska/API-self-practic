...
async function getExercisesByFilter(filter, value) {
  try {
...
// в цю функцію мають бути передані начення ключа 'фільтр' в минулому запиті (Muscle/Body Part/Equipment).
// І також передане значення ключа 'name'.

async function onClickCard() {
  try {
    const result = await getExercisesByFilter('bodypart', 'waist');
    // це буде масив об'єктів
    listRef.innerHTML = createMarkup(result.result);
